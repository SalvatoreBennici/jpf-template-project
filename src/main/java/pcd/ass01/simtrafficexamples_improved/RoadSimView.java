...
public class RoadSimView extends JFrame implements SimulationListener {
	...
	public void display() {
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				RoadSimView.this.setVisible(true);
			}
		});
	}
	...
	class RoadSimViewPanel extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			if (roads != null) {
				for (Road r : roads) {
					g2.drawLine((int) r.getFrom().getX(), (int) r.getFrom().getX(), (int) r.getTo().getX(), (int) r.getTo().getX());
				}
			}
			...
			if (sems != null) {
				for (TrafficLight s : sems) {
					...
					g2.fillRect((int) (s.getPos().getX() - 5), (int) (s.getPos().getX() - 5), 10, 10);
				}
			}
			...
			if (cars != null) {
				for (CarAgentInfo c : cars) {
					...
					g2.drawOval((int) (r.getFrom().getX() + dir.getX() - CAR_DRAW_SIZE / 2), (int) (r.getFrom().getX() + dir.getX() - CAR_DRAW_SIZE / 2), CAR_DRAW_SIZE, CAR_DRAW_SIZE);
				}
			}
		}
