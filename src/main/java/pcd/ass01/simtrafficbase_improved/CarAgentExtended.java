...
public class CarAgentExtended extends CarAgent {
	...
	private enum CarAgentState {
		STOPPED, ACCELERATING, DECELERATING_BECAUSE_OF_A_CAR, DECELERATING_BECAUSE_OF_A_NOT_GREEN_SEM,
		WAITING_FOR_GREEN_SEM, WAIT_A_BIT, MOVING_CONSTANT_SPEED
	}
	...
	public CarAgentExtended(String id, RoadsEnv env, Road road, double initialPos, double acc, double dec, double vmax) {
	...
	private boolean detectedNearCar() {
		...
		return car.filter(carAgentInfo -> carAgentInfo.getPos() - currentPercept.getRoadPos() < CAR_NEAR_DIST).isPresent();
	}

	private boolean detectedRedOrOrgangeSemNear() {
		Optional<TrafficLightInfo> sem = currentPercept.getNearestSem();
		return sem.isPresent() && !sem.get().getSem().isGreen() && sem.get().getRoadPos() - currentPercept.getRoadPos() > 0 && sem.get().getRoadPos() - currentPercept.getRoadPos() < SEM_NEAR_DIST;
	}
	...
	private boolean carFarEnough() {
		Optional<CarAgentInfo> car = currentPercept.getNearestCarInFront();
		return (!car.isPresent()) || car.get().getPos() - currentPercept.getRoadPos() > CAR_FAR_ENOUGH_DIST;
	}
}
